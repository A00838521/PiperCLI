...
## Instalación de herramientas (automática)
Usa el instalador integrado para CTF (`brew/apt/pacman/dnf`, `pip`, `go`) con detección automática.

```bash
piper ctf install --all             # instala el set recomendado
piper ctf install --tool sqlmap     # instala herramientas puntuales
piper ctf install --all --dry-run   # simula la instalación
piper ctf install --all --manager brew  # fuerza gestor concreto
```

Nota: si una instalación falla, Piper mostrará enlaces oficiales para instalación manual.
