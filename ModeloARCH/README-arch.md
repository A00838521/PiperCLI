...
```bash
# Instalar
bash ModeloARCH/install_arch.sh --model mistral:7b-instruct
bash ModeloARCH/install_arch.sh  # usa modelos por defecto (mistral, phi3)
...
### Upgrade
Actualiza Piper sin reinstalar modelos:
```bash
bash ModeloARCH/upgrade_arch.sh
bash ModeloARCH/upgrade_arch.sh --dry-run
```
Lista los modelos actuales con `ollama list` tras copiar fuentes.
