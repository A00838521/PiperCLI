#!/usr/bin/env bash
# Desinstalador de Piper CLI (modo usuario)
# - Elimina wrapper en ~/.local/bin/piper
# - Elimina copia local en ~/.local/share/piper-cli
# - Opcionalmente elimina estado en ~/.local/share/piper
# - Opcionalmente intenta detener servicio de Ollama
# Uso:
#   bash uninstall.sh [--keep-state] [--stop-service] [--dry-run]
set -euo pipefail
...
if [[ "$STOP_SERVICE" = 1 ]]; then
  if exists brew; then
    run "brew services stop ollama || true"
  fi
  if command -v launchctl >/dev/null 2>&1; then
    run "launchctl remove com.piper.ollama || true"
  fi
