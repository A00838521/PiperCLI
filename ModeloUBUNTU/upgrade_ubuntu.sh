#!/usr/bin/env bash
# Upgrade de Piper CLI para Ubuntu/Debian (ModeloUBUNTU)
# - Actualiza fuentes y mantiene modelos actuales sin hacer pull
# - Muestra lista de modelos instalados
set -euo pipefail

if [[ "$(uname -s)" != "Linux" ]]; then
  echo "[ERROR] Este upgrade es para Linux (Ubuntu/Debian)" >&2
  exit 2
fi
